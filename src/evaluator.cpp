...
//NEW DATA POINT FOR BEARING TO WP
std::map<int, double> bearing;	// this stores the current angle between the plane's actual bearing and its next waypoint
...
*/		
		// CSV version
		fprintf(fp, "%lf\n%d\n%d\n%d\n%lf", (ros::Time::now() - startTime).toSec(), waypointsTotal, numConflicts, numCollisions, totalDistTraveled/totalMinDist);
	...
	if(msg->planeID == maxAlivePlane)
	{
		...
		for(int id = 0; id <= lastPlaneID; id++)
		{
			...
			for(int otherID = 0; otherID < id; otherID++)
			{
				...
				{
					printf("%d and %d are in conflict\n", id, otherID);
