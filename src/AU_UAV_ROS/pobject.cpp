...
AU_UAV_ROS::PlaneObject::PlaneObject(int id, double bearing, double speed, double cRadius, 
	...
	this->lastUpdateTime = ros::Time::now().toSec();
	this->tMinus1Lat = 0;		// 2012 change: used to keep track of the bearing
	this->tMinus1Lon = 0;
	this->tMinus2Lat = 0;
	this->tMinus2Lon = 0;
	this->leader = -1;
...
AU_UAV_ROS::PlaneObject::PlaneObject(double cRadius, const AU_UAV_ROS::TelemetryUpdate &msg):CObject(msg.currentLatitude, msg.currentLongitude, cRadius){
	...
	this->lastUpdateTime = ros::Time::now().toSec();
	this->tMinus1Lat = 0;		// 2012 change: used to keep track of the bearing and provide waypoints for follwer planes
	this->tMinus1Lon = 0;
	this->tMinus2Lat = 0;
	this->tMinus2Lon = 0;
	this->leader = -1;
...
void AU_UAV_ROS::PlaneObject::settMinus1Lat(double latitude){
	this->tMinus1Lat = latitude;
}

void AU_UAV_ROS::PlaneObject::settMinus1Lon(double longitude){
	this->tMinus1Lon = longitude;
}

void AU_UAV_ROS::PlaneObject::settMinus2Lat(double latitude){
	this->tMinus2Lat = latitude;
}

void AU_UAV_ROS::PlaneObject::settMinus2Lon(double longitude){
	this->tMinus2Lon = longitude;
}
...
void AU_UAV_ROS::PlaneObject::setLeader(int leader){
	this->leader = leader;
}

void AU_UAV_ROS::PlaneObject::setFollower(int follower){
	this->follower = follower;
}

void AU_UAV_ROS::PlaneObject::setCheckState(bool checked){
	this->checked = checked;
}

void AU_UAV_ROS::PlaneObject::setFlock(int flock){
	this->flock = flock;
}
...
void AU_UAV_ROS::PlaneObject::update(const AU_UAV_ROS::TelemetryUpdate &msg){
	...
	// TODO - 2012 edit
	settMinus2Lat(this->gettMinus1Lat());
	settMinus2Lon(this->gettMinus1Lon());
	settMinus1Lat(this->getLatitude());
	settMinus1Lon(this->getLongitude());
	// end 2012 edit
...
double AU_UAV_ROS::PlaneObject::gettMinus1Lat(void) const{
	return this->tMinus1Lat;
}

double AU_UAV_ROS::PlaneObject::gettMinus1Lon(void) const{
	return this->tMinus1Lon;
}

double AU_UAV_ROS::PlaneObject::gettMinus2Lat(void) const{
	return this->tMinus2Lat;
}

double AU_UAV_ROS::PlaneObject::gettMinus2Lon(void) const{
	return this->tMinus2Lon;
}
...
int AU_UAV_ROS::PlaneObject::getLeader(void) const{
	return this->leader;
}

int AU_UAV_ROS::PlaneObject::getFollower(void) const{
	return this->follower;
}

bool AU_UAV_ROS::PlaneObject::getCheckState(void) const{
	return this->checked;
}

int AU_UAV_ROS::PlaneObject::getFlock(void) const{
	return this->flock;
}
