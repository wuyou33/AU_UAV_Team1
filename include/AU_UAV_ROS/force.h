...
namespace AU_UAV_ROS{
	...
	*/
	mathVector calculateForces(PlaneObject &pobj1, std::map<int, PlaneObject> &pobjects, std::vector<std::vector<AU_UAV_ROS::PlaneObject*> > flocks, AU_UAV_ROS::forceVariables forceVars);
	...
	*/
	static mathVector calculateAttractionForce(const PlaneObject &pobj1, std::map<int, PlaneObject> &pobjects);
	...
	*/
	...
	/*
	Recursively checks for followers of this plane and adds them to a vector of followers.
	In calculateForces, the vector is checked and the APF of any member of followers is ignored
	Vector of followers is limited to the calculateForces vector, does not actually belong to pobjects.
	*/
	void checkFollowers(PlaneObject &pobj1, std::map<int, PlaneObject> &pobjects, std::vector<int> &followerVect);
};
